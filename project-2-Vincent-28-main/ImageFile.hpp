...
private:
    ...
    static bool isRectangular_(const std::vector<std::vector<Pixel>>& data);

    static std::vector<std::vector<Pixel>> makeDefaultBlackGrid_();
public:
    ImageFile(const std::string& filename, const std::vector<std::vector<Pixel>>& pixels = std::vector<std::vector<Pixel>>(10, std::vector<Pixel>(20, Pixel{0, 0, 0})), const bool& readable = true, const bool& writable = true);
